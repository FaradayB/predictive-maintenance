...
=============================================================================
 ...
 Endpoints:
   ...
   POST /api/v1/track2/alert         - risk detection + English alert
...
from predictivecare.llm import build_chains, run_track1, run_track2, FAULT_METADATA, RISK_METADATA
from predictivecare.monitoring import record_llm_query, record_track1_query, record_track2_query, start_metrics_server
from predictivecare.safety import validate_request, validate_brief, validate_alert
from predictivecare.logger import log_request
from predictivecare import config
from predictivecare.features import TRACK1_FEATURES, TRACK2_FEATURES
...
# Feature order is defined once in predictivecare.features (imported above).
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.warning(f"non-fatal persistence/logging error: {e}")
 
    # ML models
    try:
        state.track1_model = joblib.load(config.TRACK1_MODEL_PATH)
        state.track2_model = joblib.load(config.TRACK2_MODEL_PATH)
...
def list_plates(track: int = Query(default=1, ge=1, le=2)):
    """
...
def alert(request: AlertRequest):
    ...
    4. If risk > 0: run RAG + LLM → English Push Alert
